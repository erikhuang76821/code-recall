...
function gitignoreBlock() {
  return [
    ...
    '.ai/memory/.lock/',
    '.ai/memory/.backups/',
    // Generated by `sync --codex`: the hook command embeds THIS machine's
    // absolute paths, so committing it hands teammates a hook that points at a
    // directory they do not have (and Codex fails such a hook silently).
    '.codex/hooks.json',
    GITIGNORE_END,
  ].join('\n');
}
...
function cmdSync(all, opts) {
  ...
  if (opts.codex) {
    ...
    console.log('');
    // Both prerequisites below fail SILENTLY when unmet — measured, not assumed:
    // Codex prints nothing, and the model simply gets no ledger. They are listed
    // as required steps, not footnotes, for exactly that reason.
    console.log('  Two things must be true or the hook does nothing, with no error:');
    console.log('   1. Trust the hook: start Codex, run /hooks, review and trust the coderecall entry.');
    console.log('      Codex re-asks after ANY change to the command, so re-check after moving or upgrading.');
    if (!opts.userLevel) {
      console.log('   2. Trust the PROJECT: project-level hooks are not loaded at all in an untrusted project');
      console.log('      (`--dangerously-bypass-hook-trust` does not cover this). Codex asks on first use here.');
    } else {
      console.log('   2. node must be on PATH — the hook runs `node "<script>"`. Run `coderecall doctor` to check.');
    }
    console.log('');
    console.log('  Start Codex from a directory that has .ai/memory/ — the hook reads the session cwd');
    console.log('  and exits silently elsewhere, so one registration is safe for all your projects.');
    console.log('  Verified: SessionStart injection into gpt-6-astra via `codex exec` (source=startup).');
    console.log('  NOT registered: PreCompact — precompact.js cannot parse Codex\'s rollout format yet');
    console.log('  and would write an empty snapshot; Stop is unverified under Codex. See COMPATIBILITY.md.');
    return;
  }
  if (all) {
...
function cmdDoctor() {
  ...
  // Codex hooks fail in two ways that produce NO error anywhere — an untrusted
  // hook and an untrusted project are both silent no-ops — so the only useful
  // check is to report exactly which of the preconditions we can see.
  console.log('[codex]');
  {
    const codexHome = path.join(os.homedir(), '.codex');
    const candidates = [
      { label: 'project .codex/hooks.json', file: path.join(CWD, '.codex', 'hooks.json'), project: true },
      { label: 'user ~/.codex/hooks.json', file: path.join(codexHome, 'hooks.json'), project: false },
    ];
    let found = 0;
    for (const c of candidates) {
      const raw = readFileSafe(c.file);
      if (raw === null) continue;
      let hasOurs = false;
      try { hasOurs = /coderecall|sessionstart\.js/i.test(raw); } catch (e) { hasOurs = false; }
      if (!hasOurs) continue;
      found++;
      ok(c.label + ' registers the coderecall SessionStart hook');
      // A commandWindows that starts with a quoted path is a measured silent no-op.
      try {
        const parsed = JSON.parse(raw);
        const groups = (parsed.hooks && parsed.hooks.SessionStart) || [];
        for (const g of groups) {
          for (const h of (g && g.hooks) || []) {
            const cw = h && h.commandWindows;
            if (typeof cw === 'string' && /^\s*"/.test(cw)) {
              warn('its commandWindows starts with a quoted path — Codex skips such a hook SILENTLY on Windows; re-run `coderecall sync --codex`');
            }
          }
        }
      } catch (e) { warn(c.label + ' is not valid JSON — Codex will ignore it'); }
      // Trust state: Codex records a hash per hook source in config.toml.
      const cfg = readFileSafe(path.join(codexHome, 'config.toml'));
      const key = c.file.replace(/\\/g, '\\');
      if (cfg === null) {
        console.log('  info  cannot read ~/.codex/config.toml — trust state unknown; run /hooks in Codex');
      } else if (cfg.indexOf('hooks.state') === -1 || cfg.indexOf(path.basename(path.dirname(key))) === -1) {
        warn('no trust entry found for it in ~/.codex/config.toml — until you run /hooks in Codex and trust it, the hook is skipped with no error');
      } else {
        console.log('  info  a [hooks.state] section exists; confirm this hook is the trusted one via /hooks');
      }
      if (c.project) {
        const lower = CWD.toLowerCase().replace(/\\/g, '\\');
        if (cfg !== null && cfg.toLowerCase().indexOf(lower) === -1) {
          warn('this project is not listed as trusted in ~/.codex/config.toml — project-level hooks are not loaded at all in an untrusted project');
        }
      }
    }
    if (!found) {
      console.log('  info  no coderecall hook registered for Codex (optional) — `coderecall sync --codex` adds one');
    } else {
      // The hook runs `node "<script>"`, so node must resolve in a plain shell.
      const cp2 = require('child_process');
      try {
        cp2.execSync(process.platform === 'win32' ? 'node --version' : 'command -v node',
          { stdio: ['ignore', 'pipe', 'pipe'], timeout: 5000 });
        ok('node resolves on PATH (the Codex hook command needs it)');
      } catch (e) {
        warn('node does not resolve on PATH — the Codex hook command `node "<script>"` will fail');
      }
    }
  }
...
/**
 * Register coderecall's hooks with Codex CLI (B3, first step).
 *
 * Codex 0.154 ships the same hook shape Claude Code uses — `hooks -> <Event> ->
 * [{matcher, hooks:[{type:"command", command, …}]}]` — and honours
 * `hookSpecificOutput.additionalContext` (or plain stdout) on `SessionStart`,
 * whose matcher covers `startup|resume|clear|compact`. Measured on this machine:
 * `hooks/sessionstart.js` used UNCHANGED as a Codex SessionStart hook injected
 * the digest and gpt-6-astra quoted the GOAL line back verbatim.
 *
 * Deliberately NOT registered here:
 *  - `PreCompact`: its stdin carries a Codex rollout JSONL, which
 *    `hooks/precompact.js` cannot parse (it expects Claude's transcript shape),
 *    so it would write an EMPTY snapshot and look like it worked.
 *  - `Stop`: Codex requires valid JSON or no output at all on Stop; `stop.js`
 *    satisfies that, but its side effects under Codex are not verified yet.
 * Both are follow-ups; shipping a hook that silently does nothing is worse than
 * shipping none.
 *
 * WINDOWS, MEASURED (Codex 0.154): a `commandWindows` that STARTS with a quoted
 * absolute path containing a space — the `"C:\Program Files\nodejs\node.exe"
 * "<script>"` form install.ps1 uses for Claude Code — makes Codex skip the hook
 * SILENTLY. No error, no warning; the model simply receives no context, which is
 * indistinguishable from having configured nothing. Isolated by bisection against
 * a live `codex exec`: forward or backslash script paths both work, the extra
 * `additionalContextLimit`/`statusMessage` fields both work, and only swapping in
 * the leading-quote absolute node path fails. (Running the identical string
 * through cmd.exe directly succeeds, so this is Codex's spawn path, not the shell's
 * quoting rule.) A `call "<node>" "<script>"` variant hung the session instead.
 * So: plain `node "<script>"`, which requires node on PATH — `doctor` checks that.
 */
function codexHookEntry() {
  const script = path.join(__dirname, 'hooks', 'sessionstart.js');
  ...
  return {
    ...
    hooks: [{
      ...
      command: cmd,
      commandWindows: cmd,
      // Codex spills hook output past ~2500 tokens to a temp file and shows the
      // model only a head/tail preview. A compact digest of CJK text can exceed
      // that, so ask for headroom explicitly rather than relying on the default.
      additionalContextLimit: 6000,
      timeout: 10,
      statusMessage: 'coderecall: injecting the project ledger',
    }],
  };
}

function syncCodexHooks(userLevel) {
  const dest = userLevel
    ? path.join(os.homedir(), '.codex', 'hooks.json')
    : path.join(CWD, '.codex', 'hooks.json');
  const res = mergeJsonFile(dest, (obj) => {
    if (!obj.hooks || typeof obj.hooks !== 'object' || Array.isArray(obj.hooks)) obj.hooks = {};
    const list = Array.isArray(obj.hooks.SessionStart) ? obj.hooks.SessionStart : [];
    obj.hooks.SessionStart = list;
    const ours = (g) => {
      try { return /coderecall|sessionstart\.js/i.test(JSON.stringify(g)); } catch (e) { return false; }
    };
    const idx = list.findIndex(ours);
    const entry = codexHookEntry();
    if (idx >= 0) {
      // Refresh in place: the trust hash keys off the hook's normalized content,
      // so a stale absolute path would keep firing the wrong file until noticed.
...
function cmdSelftest() {
  ...
  try {
    ...
    // by bisection against a live `codex exec`). Pin the measured-good shape.
    {
      const xdir = fs.mkdtempSync(path.join(os.tmpdir(), 'cr-codex-'));
      try {
        cp.execFileSync(process.execPath, [__filename, 'init'], { cwd: xdir, stdio: 'ignore' });
        const out = cp.execFileSync(process.execPath, [__filename, 'sync', '--codex'], { cwd: xdir, encoding: 'utf8' });
        const hooksFile = path.join(xdir, '.codex', 'hooks.json');
        const cfg = JSON.parse(fs.readFileSync(hooksFile, 'utf8'));
        const group = cfg.hooks.SessionStart[0];
        const h = group.hooks[0];
        check('B3: sync --codex writes a SessionStart hook', !!h && h.type === 'command');
        check('B3: the matcher covers compaction re-anchoring', /compact/.test(group.matcher));
        check('B3: commandWindows does NOT start with a quote (silent no-op on Codex/Windows)',
          typeof h.commandWindows === 'string' && !/^\s*"/.test(h.commandWindows));
        check('B3: the command points at sessionstart.js', /sessionstart\.js/.test(h.command));
        check('B3: an explicit additionalContextLimit is set (Codex spills past ~2500 tokens)',
          h.additionalContextLimit > 2500);
        check('B3: the output states the trust prerequisites', /\/hooks/.test(out) && /does nothing, with no error/.test(out));
        check('B3: the output is honest that PreCompact is not registered', /NOT registered: PreCompact/.test(out));
        check('B3: PreCompact and Stop are not registered', !cfg.hooks.PreCompact && !cfg.hooks.Stop);
        // Idempotent, and it preserves a user's unrelated hooks.
        const withUser = JSON.parse(fs.readFileSync(hooksFile, 'utf8'));
        withUser.hooks.Stop = [{ hooks: [{ type: 'command', command: 'echo mine' }] }];
        fs.writeFileSync(hooksFile, JSON.stringify(withUser, null, 2), 'utf8');
        cp.execFileSync(process.execPath, [__filename, 'sync', '--codex'], { cwd: xdir, encoding: 'utf8' });
        const after = JSON.parse(fs.readFileSync(hooksFile, 'utf8'));
        check('B3: re-running does not duplicate the hook', after.hooks.SessionStart.length === 1);
        check('B3: an unrelated user hook is preserved',
          !!after.hooks.Stop && /echo mine/.test(JSON.stringify(after.hooks.Stop)));
        // The generated file must not be committed: it embeds absolute paths.
        const gi = fs.readFileSync(path.join(xdir, '.gitignore'), 'utf8');
        check('B3: the generated hooks.json is gitignored (it embeds machine paths)',
          /\.codex\/hooks\.json/.test(gi));
        // doctor names the silent-failure mode rather than reporting all-clear.
        const bad = JSON.parse(fs.readFileSync(hooksFile, 'utf8'));
        bad.hooks.SessionStart[0].hooks[0].commandWindows = '"C:\\Program Files\\nodejs\\node.exe" "x.js"';
        fs.writeFileSync(hooksFile, JSON.stringify(bad, null, 2), 'utf8');
        let doc = '';
        try {
          doc = cp.execFileSync(process.execPath, [__filename, 'doctor'], { cwd: xdir, encoding: 'utf8', env: Object.assign({}, process.env, { CODE_RECALL_SETTINGS: path.join(xdir, 'no-settings.json') }) });
        } catch (e) { doc = String(e.stdout || ''); }
        check('B3: doctor flags a commandWindows that Codex would skip silently',
          /commandWindows starts with a quoted path/.test(doc));
      } finally { rmrf(xdir); }
    }
