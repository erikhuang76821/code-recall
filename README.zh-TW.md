...
```sh
# 從 npm 安裝(建議):
npm i -g @erikhuang/coderecall    # 裝完的指令就是 `coderecall`
# 或從原始碼安裝:
git clone https://github.com/erikhuang76821/code-recall && cd code-recall && npm link
```

> npm 套件名是 [`@erikhuang/coderecall`](https://www.npmjs.com/package/@erikhuang/coderecall)——加 scope 是因為 unscoped 名與一個不相關的既有套件撞名;**裝完的指令仍然是 `coderecall`**。Tagged release 在 [Releases](https://github.com/erikhuang76821/code-recall/releases) 頁。
...
```sh
coderecall <command>              # 全域安裝（步驟 0）後可用;或從 clone 跑:
