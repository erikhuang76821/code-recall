...
一份真實的 ledger,主題是*「替付款 webhook 加 idempotency」*。做到一半,context 視窗壓縮。**沒有** Code Recall 時,agent 斷線——重提你已否決的設計,或重加它剛寫好的 dedup key。**有**它時,PreCompact hook 在新 context 一開始就把這個重新注入(`coderecall digest --compact`,真實輸出、已截短):
...
agent 重錨定到當前的 `NOW` **連同它的理由**(為什麼 24h TTL),看到 blocker *和它的原因*,也看到兩條已在生效的決策——而不是重新推導或反過來牴觸它們。這是**真實 hook** 的輸出;同一份內容由 [CI 回歸測試](#-自我驗證-selftest--ci)在 Linux + Windows 上釘住。
