...
- **🛡️ Compaction 存活：** PreCompact 壓縮前快照，壓縮後自動全量重錨定——同類最強的一格，且有 CI 回歸測試背書。
- **🤝 跨工具共享：** 同一份 `.ai/memory/`，經 `AGENTS.md` 協定被各 IDE/CLI 讀取；git clone 就帶著走。
- **⚡ Token 紀律：** 每次只注入 ≤200 token 的摘要（整個帳本**不會**被塞進 context），非 Code Recall 專案零成本。
...
```sh
npm i -g coderecall                 # 發佈到 npm 後，`coderecall` 隨處可用
# 或從本地 clone（免發佈）：
...
## 📊 市場稀缺性 (Market scarcity) — 誰把「決策持久化」當核心？

「能記決策」的工具很多；但把「**讓過時決策失去影響力、讓現行決策保持影響力**」當**核心能力**的，幾乎沒有。各家通常只覆蓋其中一兩格：

| 能力 | Memory Bank* | llm-wiki / Obsidian | mem0 / supermemory | ADR tools† | **Code Recall** |
|---|---|---|---|---|---|
| 低摩擦捕捉 | ✅ | ❌ 手動 | ✅ 自動 | ❌ 手動 | ✅ CLI/MCP |
...
ADR 工具有狀態生命週期，但靠人手寫、不會浮現給 agent、也不撐 compaction；memory bank 會自動捕捉，但沒有決策狀態生命週期或影響力治理（append-only → 腐爛）；雲端記憶（mem0 / supermemory）多了語意召回，但不是本地零依賴、也沒有 ADR 式決策生命週期。**把「AI 維護的決策生命週期 + 影響力治理 + 跨 compaction 浮現」全湊在一起、又本地零依賴的，正是 Code Recall 補的無人車道。**
...
- **加權排序**：召回分數 = `BM25 × 狀態權重(accepted 1.0 / proposed 0.5 / deprecated 0.2 / superseded 0.05) × confidence × recency`。同樣命中時，**現行、高信心、近期**的決策一定排前面。
