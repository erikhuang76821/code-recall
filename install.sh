...
if ! command -v node >/dev/null 2>&1; then
    echo "[memo-star] ABORT: node not found on PATH. Install Node.js first." >&2
    exit 1
fi

# All merge logic lives in this inline node script (no apostrophes inside).
MEMO_SETTINGS="$SETTINGS_PATH" MEMO_HOOKS_DIR="$SCRIPT_DIR/hooks" MEMO_UNINSTALL="$UNINSTALL" node -e '
const fs = require("fs");
...
if (!uninstall && /["`$\n\r]/.test(hooksDir)) {
  console.error("[memo-star] ABORT: the hooks path contains shell-unsafe characters (\" ` $ or a newline):");
  console.error("  " + JSON.stringify(hooksDir));
  console.error("  Move the memo-star checkout to a path without these characters and re-run.");
  process.exit(1);
}
...
if (uninstall) {
  ...
  ["SessionStart", "PreCompact", "Stop"].forEach(function (ev) {
    ...
    // the unsafe-character guard above, the embedded path cannot break out.
    settings.hooks[ev].push({
      matcher: matchers[ev],
      hooks: [{ type: "command", command: "node " + JSON.stringify(hookScripts[ev]) }]
    });
