...
**Consequences:** Same family as the ADR-truncation fix — the tool's #1 risk (honor-system write-back) is mitigated at the read/observe layer: when a user mis-formats or append-drifts, the tool degrades loudly and still surfaces the most-recent state, instead of failing silent. Does NOT auto-fix a bloated TASK.md (consolidate still only archives `[x]` checklist items, not stale NOW/NEXT prose) — that remains manual + the new warning. This is the strongest cross-session validation datapoint so far: a real project where the headline promise broke on one format slip the tool didn't catch.
