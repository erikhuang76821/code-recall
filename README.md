...
> 發佈到 npm 後亦可免 clone 直接用：`npx memo-star init`（執行期仍零依賴）。
...
### 選配：MCP server（讓寫回變成工具呼叫，v2.0）

非 Claude Code 工具的寫回是榮譽制（agent 得自己想到改檔）。MCP server 把它變成**工具呼叫**——零依賴、stdio JSON-RPC，暴露 `read_memory` / `update_task` / `write_decision` / `write_lesson` / `search_memory`：

```jsonc
// Claude Desktop / Cursor / 任何 MCP client 設定
{
  "mcpServers": {
    "memo-star": { "command": "node", "args": ["<path>/Memo-star/memo.js", "mcp"] }
  }
}
```

檔案仍是儲存層；AGENTS.md 繼續覆蓋沒有 MCP 的工具。`write_decision` 走時序模型（標題重疊會 supersede 舊條目而非覆蓋）。

### 選配：知識畢業 + 跨專案教訓（v2.0）

```sh
node memo.js graduate            # >90 天、confidence high 的條目匯出到 docs/ai_wiki/
node memo.js graduate --global   # 另外把教訓寫進 ~/.memo-star/GLOBAL-LESSONS.md（跨專案）
```

匯出是**非破壞性**（條目留在帳本，標 `graduated:`，只匯出一次）。要讓跨專案教訓注入 digest（限 top-3）：開 `MEMO_STAR_GLOBAL_LESSONS=1`。全域目錄可用 `MEMO_STAR_GLOBAL_DIR` 改位置。

### 時序 / 矛盾 / 過期（v2.0，借鏡 supermemory）

DECISIONS/LESSONS 支援 `expires:`（到期自動遺忘）與 supersede 鏈：寫入標題重疊的決策時，舊條目標 `status: superseded` 並**保留**（看得到演化），不是靜默覆蓋。`consolidate` 把 superseded + 過期條目退役到 `archive/retired-YYYY-MM.md`。
...
在臨時專案模擬 compaction，斷言重錨定 digest 含全量 TASK body——把核心宣稱變成**可重現的回歸測試**。
