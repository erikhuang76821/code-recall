...
- `graduate [--global]` — export DECISIONS/LESSONS older than 90 days with `confidence: high` (and still active) into `docs/ai_wiki/<decisions|lessons>.md` (llm-wiki integration). `--global` also appends graduated lessons to `~/.coderecall/GLOBAL-LESSONS.md` (override the dir via `CODE_RECALL_GLOBAL_DIR`; note `os.homedir()` reads `USERPROFILE` on Windows). Non-destructive: graduated entries gain `- graduated: <date>` and are exported once.
