...
- `search <query> [--limit N] [--history]` — lifecycle-aware lexical search. **Default corpus = current truth**: TASK paragraphs + ACTIVE DECISIONS/LESSONS; superseded/deprecated/expired entries and the whole `archive/` are excluded. `--history` includes them (each labeled `[superseded]`/`[deprecated]`/`[archived]`). Ranking = BM25 × statusWeight (accepted/active 1.0, proposed 0.5, deprecated 0.2, superseded/archived 0.05) × confidenceWeight (high 1.0/med 0.7/low 0.4) × recencyWeight (1.0 ≤90d, easing to 0.6 by 1y). Tokenizes alphanumeric words + individual CJK chars. Read-only. Default N=5.
