...
- **Known implementation deviation (as of v2.10.0) — the `isError` contract above states the intent, not current behaviour.** Two paths break it: (1) `update_task` reports `Updated: ...` even when `TASK.md` is absent, because `setTaskField`'s `false` return is not checked; (2) any write reaching `fail()` calls `process.exit(1)`, terminating the long-lived server outright instead of returning `isError` — so lock contention or a missing `DECISIONS.md`/`LESSONS.md` kills the session rather than reporting to the model. Tracked as one combined defect (a write must either complete or return `isError`: no false success, no server termination); deferred until Gate1. Do not "fix" this by weakening the contract above. Purpose: close the honor-system write-back gap by making memory updates a tool call — files stay the storage layer; AGENTS.md still covers tools without MCP.
